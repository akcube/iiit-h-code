...
int dfs3(vector_int *adj, int v, int *dp, bool *vis){
...
int main(void){
	...
	for(int i=0; i<n; i++){
		destroy_int_vector(&adj[i]);
		destroy_int_vector(&adjt[i]);
		destroy_int_vector(&sccg[i]);
	}
